...
const getAllB = async () => {
  const payments = await Payment.find().sort({ _id: -1 }).limit(4000).exec();
  if (payments) return { payments };
...
const getOne = async (payment_ref) => {
  ...
  if (payment) {
    const receipt = await BalanceLog.findOne({
      business: payment.business_id,
      type: "CREDIT",
    })
      .sort({ _id: -1 })
      .exec();
...
const update = async (payment_ref, fields) => {
  let payment;
  try {
    payment = await Payment.findOneAndUpdate({ payment_ref }, fields, {
      new: true,
    }).exec();
