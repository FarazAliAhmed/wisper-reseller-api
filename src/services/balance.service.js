...
const getBalance = async (id) => {
  ...
  const balance = await Balance.findOne({ business: business_id })
    .populate("business")
    .exec();
...
const upgradeBalance = async (id) => {
  //change the unit from MB to ₦
...
const debit = async (id, debitAmount, field) => {
  ...
  if (checker < 0)
    return {
      error: true,
      status: 401,
      message: `Insufficient Balance ${wallet_name}`,
    };
