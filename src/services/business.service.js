...
const disableAccount = async (account_id) => {
  try {
    const resp = await Account.findOneAndUpdate(
      { _id: account_id },
      { active: false },
      { new: true }
    ).exec();
...
const updateAccountType = async (account_id, type) => {
  try {
    ...
    if (resp.type != "lite") {
      const megaPOne = await megaPrice.findOne({ business_id: resp._id });

      if (!megaPOne) {
        let megaPfirst = await megaPrice.findOne({});
        let ownMegaP = new megaPrice({
          business_id: resp._id,
          mtn_sme: megaPfirst.mtn_sme,
          mtn_gifting: megaPfirst.mtn_gifting,
          glo: megaPfirst.glo,
          airtel: megaPfirst.airtel,
          "9mobile": megaPfirst["9mobile"],
          gloDealer: megaPfirst.gloDealer,
        });
    ...
    return {
      account: resp,
      message: `User account type has been update to ${type}`,
    };
  } catch (err) {
